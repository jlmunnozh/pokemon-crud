...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const readPokemon = (id) => {
        ...
        const { name, generation, category, type, image } = pokemon;
        pokemonForm['name'].value = name;
        pokemonForm['generation'].value = generation;
        pokemonForm['category'].value = category;
    ...
    const updatePokemon = () => {
        ...
        pokemons = pokemons.map((element, index) => index !== pokemonId ? element : { name, generation, category, type, image });
        localStorage.setItem('pokemonsCrud', JSON.stringify(pokemons));
        readPokemons();
        pokemonForm.reset();
        pokemonId = -1;
